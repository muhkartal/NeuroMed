...
class MedExplainApp:
    ...
    # Fix for the _initialize_ui_components method in app2.py
    ...
    def _initialize_ui_components(self) -> None:
        ...
        # Medical Literature UI with fixed parameter names
        try:
            lit_ui_params = {}

            if health_data:
                lit_ui_params["health_data_manager"] = health_data  # Corrected parameter name

            if user_manager:
                lit_ui_params["user_manager"] = user_manager  # Correctly use user_manager
        ...
        # Fix for the medical_literature_ui initialization in app2.py
        # Replace the existing code in _initialize_ui_components method
        ...
        try:
            ...
            if health_data:
                lit_ui_params["health_data_manager"] = health_data  # Changed from "health_data" to "health_data_manager"

            if user_manager:
                lit_ui_params["user_manager"] = user_manager  # Add user_manager as required by the constructor
    ...
    def _add_custom_css(self) -> None:
        """Add custom CSS to the Streamlit application with professional enterprise styling."""
        try:
            # Enterprise-level dark theme styling with requested colors
            custom_css = """
            /* Base theme colors */
            :root {
                --background-color: #0e1117;
                --sidebar-color: #202324;
                --card-bg-color: #131820;
            ...
            .stApp, .st-emotion-cache-oczf7p, .st-emotion-cache-16txtl3 {
            ...
            /* Chart and figure styling */
            [data-testid="stImage"], .element-container .stPlotlyChart, .element-container canvas {
                background-color: var(--card-bg-color);
                padding: 1rem;
                border-radius: 8px;
                border: 1px solid var(--border-color);
            ...
            div[data-stale="false"] .tooltip {
                position: relative;
                display: inline-block;
                border-bottom: 1px dotted var(--text-muted);
            }

            div[data-stale="false"] .tooltip .tooltiptext {
                visibility: hidden;
                width: 200px;
                background-color: var(--sidebar-color);
                color: var(--text-color);
            ...
            div[data-stale="false"] .tooltip:hover .tooltiptext {
                visibility: visible;
                opacity: 1;
            }
    ...
    def _apply_theme(self) -> None:
        """Apply professional enterprise dark theme styling with the requested colors."""
        try:
            ...
            <style>
                ...
                .stDataFrame, [data-testid="stMetric"], .element-container .stPlotlyChart, .element-container canvas {
                    background-color: var(--card-bg-color);
                    border: 1px solid var(--border-color);
                }
    ...
    # Additional function to add professional widgets
    def create_status_card(title, value, status="neutral", description=None):
        """Create a professional status card with the enterprise theme"""
        status_colors = {
            "success": "#2ecc71",
            "warning": "#e67e22",
            "danger": "#e74c3c",
        ...
        card_html = f"""
        <div style="background-color: #131820; border-radius: 8px; padding: 1.25rem;
                    margin-bottom: 1rem; border: 1px solid #2d3748; box-shadow: 0 4px 6px rgba(0,0,0,0.2);">
            <div style="display: flex; align-items: center; margin-bottom: 0.5rem;">
                <div style="width: 8px; height: 8px; border-radius: 50%; background-color: {color}; margin-right: 8px;"></div>
                <p style="color: #a0a0a0; font-size: 0.9rem; margin: 0;">{title}</p>
            </div>
            <h3 style="font-size: 1.5rem; margin: 0 0 0.5rem 0; font-weight: 600; color: #ffffff;">{value}</h3>
            {f'<p style="color: #a0a0a0; font-size: 0.8rem; margin: 0;">{description}</p>' if description else ''}
        </div>
        """
    ...
    def render_sidebar(self) -> str:
        """
        Render the application sidebar with an enterprise-grade navigation experience.

        Returns:
            The selected page name
        """
        # Apply custom CSS for the sidebar
        # This styling makes the sidebar more professional and enterprise-ready
        custom_sidebar_css = """
        <style>
            /* Sidebar container refinements */
            section[data-testid="stSidebar"] > div {
                background: linear-gradient(180deg, #1a2c42 0%, #0c1625 100%);
                padding: 0;
                color: #f0f2f6;
            }
            ...
            /* Make standard Streamlit elements blend in */
            .sidebar-content [data-testid="stExpander"] {
                background-color: transparent !important;
                border: none !important;
            }

            .sidebar-content [data-testid="stExpander"] > div[role="button"] > div {
                background-color: rgba(255, 255, 255, 0.1) !important;
                border-radius: 4px;
            }

            /* Override Streamlit button styles in sidebar */
            .sidebar-content [data-testid="baseButton-secondary"] {
                background-color: rgba(255, 255, 255, 0.1) !important;
                color: rgba(255, 255, 255, 0.9) !important;
                border: 1px solid rgba(255, 255, 255, 0.2) !important;
            }

            /* Style radio buttons and checkboxes to blend with theme */
            .sidebar-content .st-emotion-cache-1k5zddu input {
                border-color: rgba(255, a255, 255, 0.2);
            }

            /* Add subtle horizontal divider */
            .divider {
                height: 1px;
        ...
        with st.sidebar:
            # Create an empty container to hold our custom sidebar
            sidebar_container = st.container()

            # Use this empty container for all Streamlit interactions
            # that we need but don't want visible in the UI
            hidden_container = st.container()

            with hidden_container:
                # Create category selector with horizontal radio buttons
                # This is hidden from the UI but we need it for state
                categories = {
            ...
            if SessionManager.get("advanced_mode", False):
                categories["System"].append("Admin Panel")

            # Create radio selector for categories
            ...
            # Format page names with icons
            pages_with_icons = [f"{page_icons.get(page, '•')} {page}" for page in categories[selected_category]]

            # Create a selection for pages in the current category
            selected_page_with_icon = st.radio(
                f"{selected_category} Pages",
                pages_with_icons,
                label_visibility="collapsed",
                key=f"nav_page_{selected_category}"
            )

            # Safely remove the icon to get the actual page name
            if " " in selected_page_with_icon:
                emoji, _, raw_page_name = selected_page_with_icon.partition(" ")
                selected_page = raw_page_name.strip()
            else:
                selected_page = selected_page_with_icon

            # Set the page in session state for navigation
            SessionManager.set("page", selected_page)

            # Now we can build our custom sidebar UI
            with sidebar_container:
                # Add class to identify our sidebar content for CSS targeting
                ...
                <div class="sidebar-header">
                    <div class="logo-container">
                        ...
                        <div>
                            <h1 class="app-title">MedExplain AI Pro</h1>
                            <p class="app-version">Enterprise Edition v{}</p>
                        </div>
                    </div>
                </div>
                """.format(AppConfig.VERSION), unsafe_allow_html=True)
                ...
                # Category tabs
                category_html = '<div class="category-tabs">'
                for category in categories.keys():
                    active_class = "active" if category == selected_category else ""
                    category_html += f'<div class="category-tab {active_class}" onclick="document.querySelector(\'[data-testid="stRadioButton-Categories-{category}"]\').click();">{category}</div>'
                category_html += '</div>'
                st.markdown(category_html, unsafe_allow_html=True)

                # Current category pages
                st.markdown(f'<div class="nav-menu">', unsafe_allow_html=True)

                for i, page in enumerate(categories[selected_category]):
                    ...
                    <div class="nav-item {active_class}"
                        onclick="document.querySelector('[data-testid=\"stRadioButton-{selected_category} Pages-{icon} {page}\"]').click();">
                ...
                # Quick Check Action
                quick_check_col, health_report_col = st.columns(2)

                with quick_check_col:
                    quick_check = st.button(
                        "🔍 Quick Check",
                        use_container_width=True,
                        help="Quickly analyze your symptoms"
                    )
                    if quick_check:
                        SessionManager.navigate_to("Symptom Analyzer")
                        return "Symptom Analyzer"

                with health_report_col:
                    health_report = st.button(
                        "📋 Health Report",
                        use_container_width=True,
                        help="Generate a comprehensive health report"
                    )
                    if health_report:
                        risk_assessor = self.registry.get("risk_assessor")
                ...
                if notification_count > 0:
                    st.markdown(f"""
                    <div class="notification-badge" onclick="document.querySelector('[data-testid=\"baseButton-View Notifications\"]').click();">
                        <div>📬 {notification_count} Notification{'' if notification_count == 1 else 's'}</div>
                    </div>
                    """, unsafe_allow_html=True)

                    # Hidden button that gets triggered by the custom notification badge
                ...
                if user_manager and user_manager.profile:
                    ...
                    # Prepare profile HTML
                    profile_html = '<div class="profile-card">'

                    if profile.get("name"):
                        profile_html += f'<div class="profile-name">{profile["name"]}</div>'

                    # Create a row for age and gender
                    profile_html += '<div class="profile-details">'

                    if profile.get("age"):
                        profile_html += f'<div style="flex: 1;"><strong>Age:</strong> {profile["age"]}</div>'

                    if profile.get("gender"):
                        profile_html += f'<div style="flex: 1;"><strong>Gender:</strong> {profile["gender"]}</div>'
                    ...
                    if last_risk_assessment:
                        ...
                        profile_html += f'<div><strong>Status:</strong> <span style="color:{risk_color}; font-weight: 500;">{risk_level.capitalize()}</span></div>'

                    # Add edit profile button that triggers the Streamlit button
                    profile_html += '</div>'

                    # Render the profile card
                    st.markdown(profile_html, unsafe_allow_html=True)

                    # Hidden button that gets triggered by the custom edit button
                    if st.button("Edit Profile", use_container_width=True, key="edit_profile_btn"):
                        SessionManager.navigate_to("Settings")
                        return "Settings"

                # Application settings section with enhanced styling
                st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
                ...
                with st.expander("Application Settings", expanded=False):
                    ...
                    new_advanced_mode = st.toggle("Advanced Mode", value=advanced_mode)

                    if new_advanced_mode != advanced_mode:
                        SessionManager.set("advanced_mode", new_advanced_mode)
                        st.rerun()

                    # View mode selector (only in advanced mode)
                ...
                <div class="sidebar-footer">
                    ...
                    <p style="margin-top: 10px;">© {0} MedExplain AI Pro · v{1}</p>
                </div>
                """.format(datetime.now().year, AppConfig.VERSION), unsafe_allow_html=True)

                # Close the sidebar-content div
                st.markdown('</div>', unsafe_allow_html=True)

        # Return the selected page for the app to display
    ...
    def navigate_to(page: str) -> None:
        ...
        # Force a rerun to reflect the navigation change
        st.rerun()
    ...
    def run(self) -> None:
        ...
        try:
            ...
            # Get the page from session state, which will be set by the sidebar navigation
            page_from_session = SessionManager.get("page")

            # Use the page from session state if available
            if page_from_session:
                selected_page = page_from_session
        ...
        try:
            ...
            with col2:
                ...
                if image_path.exists():
                    ...
                    st.image("../NeuroMed/img/hero.png" , width=3)
