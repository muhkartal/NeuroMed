...
def render_symptom_analyzer() -> None:
    ...
    # Check if required components are in session state
    if "health_data_manager" not in st.session_state or "user_manager" not in st.session_state:
    ...
    with tabs[0]:
        _render_selection_method(health_data_manager, user_manager, openai_client)

    with tabs[1]:
        _render_text_description(health_data_manager, user_manager, openai_client)

    with tabs[2]:
        _render_voice_input(health_data_manager, user_manager, openai_client)

def _render_selection_method(health_data_manager, user_manager, openai_client) -> None:
    """
...
def _render_text_description(health_data_manager, user_manager, openai_client) -> None:
    """
...
def _render_voice_input(health_data_manager, user_manager, openai_client) -> None:
    """
...
def _process_symptom_analysis(health_data_manager, user_manager, openai_client,
    ...
    with st.spinner("Analyzing your symptoms with advanced AI models..."):
        try:
            ...
            # Display results
            _display_symptom_analysis_results(analysis_results, risk_assessment)
...
def _process_text_analysis(health_data_manager, user_manager, openai_client, symptom_text) -> None:
    """
    ...
    with st.spinner("Analyzing your symptom description with NLP..."):
        try:
            ...
            # Display results
            _display_symptom_analysis_results(analysis_results, None)
...
def _display_symptom_analysis_results(analysis_results, risk_assessment=None) -> None:
    """
...
def _display_ai_explanation(openai_client, health_data_manager, selected_symptoms, additional_data) -> None:
    """
